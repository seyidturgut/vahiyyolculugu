...
export const LaserFlow: React.FC<LaserFlowProps> = ({
    ...
    useEffect(() => {
        ...
        const clampLevel = (v: number, lo: number, hi: number) => Math.max(lo, Math.min(hi, v));
        const dprFloor = 0.6;
        ...
        const adjustDprIfNeeded = (now: number) => {
            ...
            if (avgFps < lowerThresh) {
                next = clampLevel(currentDprRef.current * 0.85, dprFloor, base);
            } else if (avgFps > upperThresh && currentDprRef.current < base) {
                next = clampLevel(currentDprRef.current * 1.1, dprFloor, base);
            }
